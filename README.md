...
* TWEAK: Register the setting section and field for the "Uninstall" setting option. Trigger a browser prompt form
  confirmation.
...
* TWEAK: Move the `Connections_Directory()` global function from the `Connections_Directory` class to the base plugin
  file.
* TWEAK: Move the Activate/Deactivate hooks to their own class/file.
* TWEAK: Refactor `uninstall.php` to delete the plugin data, options, and tables if the option is enabled in the
  settings.
...
* BUG: Remove empty strings from the page title pieces to prevent leading/trailing page title separators from
  displaying.
* OTHER: Correct misspellings.
* DEV: phpDoc corrections.
...
* WPCS: Disable `PEAR.NamingConventions.ValidClassName.StartWithCapital`
  and `WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound`.
...
* TWEAK: Utilize the `wpseo_robots_array` filter to improve Yoast SEO integration by setting `noindex` on the pg and
  term slug pages.
...
* TWEAK: Rename the `redirect` property to `redirect_to` in the `account` REST API route to match the core WordPress
  naming convention.
...
* TWEAK: Change the priority of the `parse_request` from `10` to `11` to stop Event Espresso from breaking the ability
  to parse the request variables.
