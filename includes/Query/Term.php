...
use function Connections_Directory\Taxonomy\_padTermCounts as _pad_term_counts;
...
use function Connections_Directory\Taxonomy\updateTermMetaCache as update_termmeta_cache;
use function Connections_Directory\Utility\_deprecated\_applyFilters as apply_filters_deprecated;

/**
 * Class Term
 *
 * NOTE: This is the Connections equivalent of @see \WP_Term_Query in WordPress core ../wp-includes/class-wp-term-query.php
 *
...
final class Term {
	 ...
	 *
	 * @since 10.3
	 * @var cnMeta_Query A meta query instance.
	 */
	...
	public function parse_query( $query = '' ) {
		 ...
		 */
		$this->query_var_defaults = apply_filters( 'Connections_Directory/Query/Term/Default_Parameters', $this->query_var_defaults, $taxonomies );
		 ...
		 * @since 10.3
		 *
		 * @param Term $this Current instance of Term.
		 */
		do_action( 'Connections_Directory/Query/Term/Parse_Query', $this );
	}
	...
	public function get_terms() {
		...
		$this->meta_query = new cnMeta_Query();
		 ...
		 * @since 10.3
		 *
		 * @param Term $this Current instance of WP_Term_Query (passed by reference).
		 */
		do_action_ref_array( 'Connections_Directory/Query/Term/Get_Terms/Before', array( &$this ) );
		 ...
		 * @param string[] $taxonomies An array of taxonomy names.
		 */
		$args = apply_filters( 'Connections_Directory/Query/Term/Get_Terms/Properties', $args, $taxonomies );
		...
		if ( $orderby ) {
			 ...
			 * @param string[] $taxonomies A taxonomy or array of taxonomies.
			 */
			$orderby = apply_filters_deprecated(
				'cn_terms_orderby',
				array( "ORDER BY $orderby", $args, $taxonomies ),
				'10.3',
		...
		if ( ! empty( $inclusions ) ) {
			 ...
			 * @param string[] $taxonomies A taxonomy or array of taxonomies.
			 */
			$this->sql_clauses['where']['inclusions'] = apply_filters_deprecated(
				'cn_term_inclusions',
				array( 't.term_id IN ( ' . $inclusions . ' )', $args, $taxonomies ),
				'10.3',
		 ...
		 * @param string[] $taxonomies An array of taxonomy names.
		 */
		$exclusions = apply_filters( 'Connections_Directory/Query/Term/Get_Terms/Exclude_Terms_Clause', $exclusions, $args, $taxonomies );
		 ...
		 * @since 10.3
		 *
		 * @param array|null    $terms Return an array of term data to short-circuit the term query,
		 *                             or null to allow queries to run normally.
		 * @param Term          $query The \Connections_Directory\Query\Term instance, passed by reference.
		 */
		$this->terms = apply_filters_ref_array( 'Connections_Directory/Query/Term/Get_Terms/Pre_Query', array( $this->terms, &$this ) );
		...
		$last_changed = wp_cache_get_last_changed( 'cn_terms' );
		...
		if ( 'count' === $_fields ) {
			...
			wp_cache_set( $cache_key, $count, 'cn_terms' );
			return $count;
		}
		...
		if ( $hierarchical && $args['hide_empty'] && is_array( $terms ) ) {
			foreach ( $terms as $k => $term ) {
				if ( ! $term->count ) {
					$children = cnTerm::children( $term->term_id, $term->taxonomy );
					if ( is_array( $children ) ) {
						foreach ( $children as $child_id ) {
							$child = cnTerm::get( $child_id, $term->taxonomy );
							if ( $child->count ) {
								continue 2;
							}
						}
	...
	protected function parse_orderby( $orderby_raw ) {
		 ...
		 * @param string[] $taxonomies An array of taxonomy names.
		 */
		$orderby = apply_filters( 'Connections_Directory/Query/Term/Get_Terms/Order_By', $orderby, $this->query_vars, $this->query_vars['taxonomy'] );
