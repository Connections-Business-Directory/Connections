...
class cnRegisterSettings {
	...
	public function registerSettingsSections( $sections ) {
		...
		$sections[] = array(
			...
			'callback'  => create_function( '', "_e('Choose the page where your directory is located. This should be the page where you used the [connections] shortcode.', 'connections');" ),
			'page_hook' => $settings
		);
		$sections[] = array(
			'tab'       => 'general',
		...
		$sections[] = array(
			...
			'callback'  => create_function( '', "_e('The [connections] shortcode has two options available to show an entryor an entire directoryif the entry(ies) has been set to private or the user is required to be logged to view the directory. These options, when used, will only be applied to the current shortcode instance.', 'connections');" ),
			'page_hook' => $settings
		);
		...
		$sections[] = array(
			...
			'callback'  => create_function( '', "_e('When an image or logo is uploaded to an entry, various sizes are created and cached. This helps to reduce server load during therendering of the directory. If these settings are changed, they will only affect images uploaded after the change has been made. All previous images will remain at their previously cached sizes. NOTE: the active template will determine which image(s) is used.', 'connections');" ),
			'page_hook' => $settings
		);
		$sections[] = array(
			'tab'       => 'images',
	...
	public function registerSettingsFields( $fields ) {
		...
		$fields[] = array(
			...
			'desc'      => __( 'Enabling this option will turn the name ofevery entry into a link. Clicking the link will take you to the page with only that entry.', 'connections' ),
			'help'      => '',
			'type'      => 'checkbox',
			'default'   => 0
		);
