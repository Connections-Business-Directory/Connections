<?php
/**
 * The batch export the email addresses as a CSV file.
 *
 * @package     Connections
 * @subpackage  CSV Batch Export Email
...
/**
 * cnCSV_Batch_Export_Email Class
 *
 * @since 8.5
 */
class cnCSV_Batch_Export_Email extends cnCSV_Batch_Export {
	...
	public function getData() {
		...
		$sql = $wpdb->prepare(
			'SELECT SQL_CALC_FOUND_ROWS entry.id, entry.entry_type, entry.family_name, entry.honorific_prefix, entry.first_name, entry.middle_name, entry.last_name, entry.honorific_suffix, entry.organization, email.*
			 FROM ' . CN_ENTRY_TABLE . ' AS entry
			 INNER JOIN ' . CN_ENTRY_EMAIL_TABLE . ' AS email
			 ON entry.id = email.entry_id
			 WHERE 1=1
			 AND email.address != ""
			 ORDER BY email.entry_id, email.order
			 LIMIT %d
			 OFFSET %d',
			$this->limit,
			absint( $offset )
