...
class DeleteConstrainer implements Plugin {
	...
	public function registered( Table $table ) {
		...
		foreach ( $constraints as $column_name => $behavior ) {
			...
			/* Renamed $model as $model_option. For PHP 7.1. */
			$column->register_delete_callback( function ( $pk, $model_option ) use ( $table, $column_name, $model, $behavior ) {

				/** @var FluentQuery $query */
				$query = call_user_func( array( $model_option, 'query' ) );
